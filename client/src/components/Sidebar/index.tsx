...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  return (
    ...
    >
      {/* <!-- SIDEBAR HEADER --> */}
      <div className="flex items-center justify-between w-full gap-1 px-6 pt-12">
        <div className="flex items-center">
          <Link href="/">
            <Image
              width={60}
              height={32}
              src={"/images/png/funny-loader.png"}
              alt="Logo"
            />
          </Link>
          <p className="text-lg font-bold text-white dark:text-body">ExTrack</p>
        </div>
        {
          sidebarOpen &&
          <div className="text-2xl font-bold flex items-center cursor-pointer" onClick={() => setSidebarOpen(false)}>
            x
          </div>
        }
      </div>
        ...
        <nav className="mt-5 py-4 px-4 lg:mt-9 lg:px-6">
          ...
          <div>
            ...
            <ul className="mb-6 flex flex-col gap-1.5">
              ...
              <SidebarLinkGroup
                activeCondition={
                  ["/", '/expense', '/savings', '/income', '/details'].includes(pathname)
                }
              >
                {(handleClick, open) => {
                  return (
                    <React.Fragment>
                      <Link
                        href="#"
                        className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-primarydark dark:hover:bg-meta-4 ${["/", '/expense', '/savings', '/income', '/details'].includes(pathname) &&
